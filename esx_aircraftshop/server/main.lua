...
TriggerEvent('esx_phone:registerNumber', 'aircraftshop', _U('air_customers'), false, false)
TriggerEvent('esx_society:registerSociety', 'aircraftshop', _U('air_dealer'), 'society_aircraftshop', 'society_aircraftshop', 'society_aircraftshop', {type = 'private'})
...
RegisterNetEvent('esx_aircraftshop:setVehicleOwnedPlayerId')
AddEventHandler('esx_aircraftshop:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)

	if xPlayer.job.name == 'Aircraftshop' and xTarget then
		MySQL.Async.fetchAll('SELECT id FROM aircraft_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			['@vehicle'] = model
		}, function(result)
			if result[1] then
...
ESX.RegisterServerCallback('esx_aircraftshop:getSoldVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT client, model, plate, soldby, date FROM vehicle_sold', {}, function(result)
		cb(result)
	end)
end)

RegisterNetEvent('esx_aircraftshop:rentVehicle')
AddEventHandler('esx_aircraftshop:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)

	if xPlayer.job.name == 'Aircraftshop' and xTarget then
		MySQL.Async.fetchAll('SELECT id, price FROM aircraft_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			['@vehicle'] = vehicle
		}, function(result)
			if result[1] then
...
RegisterNetEvent('esx_aircraftshop:getStockItem')
AddEventHandler('esx_aircraftshop:getStockItem', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)

	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_sircraftshop', function(inventory)
...
RegisterNetEvent('esx_aircraftshop:putStockItems')
AddEventHandler('esx_aircraftshop:putStockItems', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)

	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_aircraftshop', function(inventory)
		local item = inventory.getItem(itemName)
...
ESX.RegisterServerCallback('esx_aircraftshop:getCommercialVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT price, vehicle FROM aircraft_vehicles ORDER BY vehicle ASC', {}, function(result)
		cb(result)
	end)
end)

ESX.RegisterServerCallback('esx_aircraftshop:buyAircraftshopVehicle', function(source, cb, model)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'Aircraftshop' then
		local modelPrice
		...
		if modelPrice then
			TriggerEvent('esx_addonaccount:getSharedAccount', 'society_aircraftshop', function(account)
...
ESX.RegisterServerCallback('esx_aircraftshop:giveBackVehicle', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT base_price, vehicle FROM rented_vehicles WHERE plate = @plate', {
		...
		if result[1] then
			...
			MySQL.Async.execute('DELETE FROM rented_aircraft WHERE plate = @plate', {
				['@plate'] = plate
			}, function(rowsChanged)
				MySQL.Async.execute('INSERT INTO aircraft_vehicles (vehicle, price) VALUES (@vehicle, @price)', {
					['@vehicle'] = vehicle,
...
ESX.RegisterServerCallback('esx_aircraftshop:resellVehicle', function(source, cb, plate, model)
	...
	if xPlayer.job.name == 'aircraftshop' then
		...
		if not resellPrice then
			print(('[esx_aircraftshop] [^3WARNING^7] %s attempted to sell an unknown vehicle!'):format(xPlayer.identifier))
			cb(false)
		else
...
ESX.RegisterServerCallback('esx_aircraftshop:isPlateTaken', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT 1 FROM owned_aircrafts WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
		cb(result[1] ~= nil)
	end)
...
function UnrentVehicleAsync(identifier, plate)
	MySQL.Async.execute('DELETE FROM rented_aircraft WHERE identifier = @identifier AND plate = @plate', {
...
function PayRent(d, h, m)
	...
	print('[esx_aircraftshop] [^2INFO^7] Paying rent cron job started')
	...
	MySQL.Async.fetchAll('SELECT owner, rent_price, plate FROM rented_vehicles', {}, function(result)
		...
		print(('[esx_aircraftshop] [^2INFO^7] Paying rent cron job took %s seconds'):format(elapsedTime))
	end)
end
