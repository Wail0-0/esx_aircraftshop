...
Citizen.CreateThread(function()
	...
	if Config.EnablePlayerManagement then
		if ESX.PlayerData.job.name == 'airdealer' then
			Config.Zones.AirEntering.Type = 1
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	if Config.EnablePlayerManagement then
		if ESX.PlayerData.job.name == 'dealer' then
			Config.Zones.AirEntering.Type = 1
...
function OpenShopMenu()
	...
	SetEntityCoords(playerPed, Config.Zones.AirInside.Pos)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('esx_aircraftshop:buyAirCraftDealerVehicle', function(success)
						if success then
							IsInShopMenu = false
							...
							SetEntityCoords(playerPed, Config.Zones.AirEntering.Pos)
					...
					ESX.TriggerServerCallback('esx_aircraftshop:buyVehicle', function(success)
						if success then
							...
							ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.AirOutside.Pos, Config.Zones.AirOutside.Heading, function(vehicle)
		...
		ESX.Game.SpawnLocalVehicle(vehicleData.model, Config.Zones.AirInside.Pos, Config.Zones.AirInside.Heading, function(vehicle)
	...
	ESX.Game.SpawnLocalVehicle(firstVehicleData.model, Config.Zones.AirInside.Pos, Config.Zones.AirInside.Heading, function(vehicle)
		currentDisplayVehicle = vehicle
		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
		FreezeEntityPosition(vehicle, true)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_aircraftshop', _U('society_aircraftshop'), tonumber(data2.value))
						end
					end
				end, function(data2, menu2)
					menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					TriggerServerEvent('esx_aircraftshop:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, CurrentVehicleData.model, CurrentVehicleData.name)
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
						...
						if not amount then
							...
							if closestPlayer ~= -1 and closestDistance < 3 then
								...
								SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
								TriggerServerEvent('esx_aircraftshop:rentVehicle', model, newPlate, amount, GetPlayerServerId(closestPlayer))
								currentDisplayVehicle = nil
							else
								ESX.ShowNotification(_U('no_players'))
							end
...
function OpenPopVehicleMenu()
	ESX.TriggerServerCallback('esx_aircraftshop:getCommercialVehicles', function(vehicles)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'commercial_vehicles', {
			...
			ESX.Game.SpawnVehicle(model, Config.Zones.AirInside.Pos, Config.Zones.AirInside.Heading, function(vehicle)
				currentDisplayVehicle = vehicle
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'aircraftshop', function(data2, menu2)
				menu2.close()
			end)
		elseif data.current.value == 'sold_vehicles' then

			ESX.TriggerServerCallback('esx_aircraftshop:getSoldVehicles', function(customers)
				local elements = {
...
AddEventHandler('esx:setJob', function(job)
	...
	if Config.EnablePlayerManagement then
		if ESX.PlayerData.job.name == 'aircraftshop' then
			Config.Zones.AirEntering.Type = 1
...
AddEventHandler('esx_aircraftshop:hasEnteredMarker', function(zone)
	if zone == 'AirEntering' then

		if Config.EnablePlayerManagement then
			if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'aircraftshop' then
				CurrentAction     = 'reseller_menu'
				CurrentActionMsg  = _U('shop_menu')
				CurrentActionData = {}
			end
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'aircraftshop' and ESX.PlayerData.job.grade_name == 'boss' then
		CurrentAction     = 'boss_actions_menu'
		CurrentActionMsg  = _U('shop_menu')
		CurrentActionData = {}
	end
...
-- Create Blips
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(Config.Zones.AirEntering.Pos)
	...
	AddTextComponentSubstringPlayerName(_U('air_dealer'))
